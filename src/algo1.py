...
def evaluator(particle: Particle, map: Map) -> int:
    """
    ...
    return norm_distance * distance * math.tan(normalization_factor) - res_count * norm_resources


def compute_all_map(map: Map):
    result = np.zeros(map.map_dim)
    particle = Particle((0, 0), 0, (0, 0), RESOURCE_RADIUS)
    for i in range(0, map.map_dim[0]):
        for j in range(0, map.map_dim[1]):
            particle.current_position = (i, j)
            result[i][j] = evaluator(particle, map)
...
if __name__ == "__main__":
    ...
    particle = generate(map, resource_half_square=RESOURCE_RADIUS,
                        random=rand,
    ...
    if sys.argv[1] == "all":
        ...
        x = int(input("X coord of your point?"))
        y = int(input("Y coord of your point?"))
        print("X: ", str(x))
        print("Y: ", str(y))

        res = evaluator(particle, map)
        print("Value of function in this point: " + str(res))
