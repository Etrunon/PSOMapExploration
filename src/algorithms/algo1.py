...
class Algo1(Algorithm):
    ...
    def generate_particle(self, random: Random, args) -> Particle:
        ...
        while True:
            random_point = (random.randint(self.resource_range, world_map.world_map.map_dim[0] - self.resource_range),
                            random.randint(self.resource_range, world_map.world_map.map_dim[1] - self.resource_range))

            # If there is earth under the chosen point break outside, else generate another point
            if world_map.world_map.water_map[random_point[0]][random_point[1]] == 0:  # 0 means earth in the matrix
                break
    ...
    def compute_score(self, particle: Particle) -> float:
        ...
        normalization_factor = math.atan(world_map.world_map.map_dim[0] / square_area)
    ...
    def __init__(self, maximum_velocity: int, resource_range: int, save_history: bool) -> None:
        super().__init__(resource_range)
    ...
    def get_fitness_landscape(self) -> np.ndarray:
        ...
        """
        landscape = np.zeros(world_map.world_map.map_dim)

        for x in range(self.resource_range, world_map.world_map.map_dim[0] - self.resource_range):
            for y in range(self.resource_range, world_map.world_map.map_dim[1] - self.resource_range):
                sensor = Particle((x, y), np.array((1, 1), np.uintc), CITY_POSITION, self.resource_range, 0, False)
